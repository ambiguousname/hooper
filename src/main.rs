...
async fn main() -> Result<(), Box<dyn std::error::Error>>{
	...
	let cert_info = if let Some(c) = args.cert {
		let certs = CertificateDer::pem_file_iter(format!("{c}.pem"))?
	...
	let tls_acceptor = if let Some((certs, key)) = cert_info {
		let mut server_config = ServerConfig::builder().with_no_client_auth().with_single_cert(certs, key).map_err(|e| {
	...
	loop {
		...
		tokio::spawn(async move {
			if let Some(a) = tls_acceptor {
				...
				if let Err(e) = Builder::new(TokioExecutor::new()).serve_connection(TokioIo::new(st), service).await {
					eprintln!("Failed to serve connection: {e:#}");
				}
			} else {
				if let Err(e) = Builder::new(TokioExecutor::new()).serve_connection(TokioIo::new(tcp_stream), service).await {
...
static MEMBERS : LazyLock<Vec<Member>> = LazyLock::new(|| {
	get_members().expect("Could not compile members list.")
});
...
fn redirect_to_member<T>(idx : usize, response : &mut Response<T>) -> Result<&Member, String> {
	*response.status_mut() = StatusCode::SEE_OTHER;
	...
	let val : HeaderValue = new_member.url.to_string().parse().map_err(|e : hyper::header::InvalidHeaderValue| {
		e.to_string()
	})?;
...
fn redirect_from_referer<T>(referer : Option<&HeaderValue>, add : isize, response : &mut Response<T>) -> Result<String, String> {
	let new_member = if let Some(r) = referer {
		let url_st = r.to_str().map_err(|e| {
			e.to_string()
		})?;
		let uri = Uri::from_str(url_st).map_err(|e| {
			e.to_string()
		})?;

		let member = MEMBERS.iter().find(|member| {
			member.url.authority() == uri.authority()
		});
